...
export default function App() {
  ...
  useEffect(() => {
    try {
      ...
      if (typeof parsed?.panicMode === "boolean")
        setPanicMode(parsed.panicMode);
  ...
  const data = useMemo(() => {
    return stations.map((station, idx) => {
      ...
      const prevActualOut =
        idx > 0
          ? records[idx - 1].actualOut
          : records[0].actualOut || records[0].actualIn;
  ...
  const activeIndex = data.findIndex(
    (d, i) =>
      !d.actualOut &&
      (i === 0 || !!data[i - 1].actualOut || !!data[i - 1].actualIn)
  );
  const currentIndex = activeIndex === -1 ? data.length - 1 : activeIndex;
  const current = data[currentIndex];
  const nextCritical = data.find(
    (d, i) => i >= currentIndex && d.critical && !d.actualOut
  );
  const overallDelta = data.reduce(
    (acc, d) => (d.deltaMs !== null ? acc + d.deltaMs : acc),
    0
  );
  ...
  function stamp(index, field) {
    ...
    setRecords((prev) =>
      prev.map((r, i) => (i === index ? { ...r, [field]: now } : r))
    );
  }
  ...
  function clearStation(index) {
    setRecords((prev) =>
      prev.map((r, i) =>
        i === index ? { actualIn: "", actualOut: "", note: "" } : r
      )
    );
  }
  ...
  function importData() {
    try {
      ...
      if (parsed?.records?.length === stations.length)
  ...
  const status = current
    ? statusFromDelta(current.deltaMs)
    : statusFromDelta(null);
      ...
      <div style={styles.wrap}>
          ...
          <div style={styles.card}>
            ...
            <div style={{ ...styles.grid2, marginTop: "12px" }}>
              <div style={styles.statBox}>
                ...
                <div style={{ fontWeight: 800 }}>
                  {formatDateTime(current.plannedIn)}
                </div>
              </div>
              <div style={styles.statBox}>
                <div style={styles.muted}>Leave by</div>
                <div style={{ fontWeight: 800 }}>
                  {formatDateTime(current.plannedOut)}
                </div>
              </div>
              <div style={styles.statBox}>
            ...
            <div style={{ ...styles.grid2, marginTop: "12px" }}>
              <div style={styles.statBox}>
                ...
                <div style={{ fontWeight: 800 }}>
                  {formatDateTime(current.actualIn)}
                </div>
              </div>
              <div style={styles.statBox}>
                <div style={styles.muted}>Actual out</div>
                <div style={{ fontWeight: 800 }}>
                  {formatDateTime(current.actualOut)}
                </div>
              </div>
              <div style={styles.statBox}>
                <div style={styles.muted}>Segment delta</div>
                <div style={{ fontWeight: 800 }}>
                  {formatDuration(current.deltaMs)}
                </div>
              </div>
              <div style={styles.statBox}>
                <div style={styles.muted}>Stop time</div>
                <div style={{ fontWeight: 800 }}>
                  {formatDuration(current.stopMs)}
                </div>
              </div>
            </div>
            ...
            >
              ...
              <div
                style={{ marginTop: "8px", fontSize: "14px", fontWeight: 700 }}
              >
                {panicMode
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      <div>
                        Out:{" "}
                        <strong>{formatTimeOnly(station.plannedOut)}</strong>
                      </div>
                      <div>
                        Actual in:{" "}
                        <strong>{formatTimeOnly(station.actualIn)}</strong>
                      </div>
                      <div>
                        Actual out:{" "}
                        <strong>{formatTimeOnly(station.actualOut)}</strong>
                      </div>
                    </div>
                    <div style={{ ...styles.grid2, marginTop: "10px" }}>
                      <button
          ...
          <div style={styles.card}>
            ...
            <div style={{ display: "grid", gap: "10px", marginTop: "12px" }}>
              ...
              <div style={styles.statBox}>
                ...
                <div
                  style={{ ...styles.muted, marginTop: "6px", lineHeight: 1.6 }}
                >
        ...
        {tab === "backup" && (
          <div style={styles.card}>
            ...
            <div style={{ ...styles.muted, marginTop: "6px" }}>
              Use this before crew phone handoffs or if you want a second-device
              backup.
            </div>
            <div style={{ ...styles.grid2, marginTop: "12px" }}>
              <button
